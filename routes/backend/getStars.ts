...
router.route('/' + routeName + '/')
    .post(getUserPOST, requireArgumentsPost(["path", "token"]), postRouteHandler);

function postRouteHandler(req:express.Request, res:express.Response) {
    res.send(fileStuff.getStars(req.body.token, req.header('x-forwarded-for') || req.socket.remoteAddress, req.body.path))
}
